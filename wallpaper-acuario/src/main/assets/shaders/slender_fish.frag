...
void main() {
    ...
    // Slender Fish shape
    float aBody = ellipseAlpha(pWarped, vec2(0.08, 0.0), vec2(0.50, 0.13), 0.025);
    float aDorsalFin = ellipseAlpha(pWarped, vec2(-0.08, 0.13), vec2(0.20, 0.05), 0.025);
    float aVentralFin = ellipseAlpha(pWarped, vec2(-0.12, -0.13), vec2(0.15, 0.04), 0.025);
    
    float tailLen = 0.12;
    float tX = (pWarped.x - (-0.35)) / (-tailLen);
    float yNorm = pWarped.y / (0.03 + 0.10 * clamp(tX, 0.0, 1.0));
    float tXLimit = 0.6 + 0.4 * (yNorm * yNorm);
    float xBounds = smoothstep(0.0, 0.05, tX) * smoothstep(tXLimit, tXLimit - 0.05, tX);
    float halfHeight = 0.03 + 0.10 * clamp(tX, 0.0, 1.0);
    float yBounds = smoothstep(halfHeight, halfHeight - 0.025, abs(pWarped.y));
    float aTailFin = xBounds * yBounds;
    
    float aEye = ellipseAlpha(pWarped, vec2(0.38, 0.04), vec2(0.032, 0.032), 0.015);
    float aPupil = ellipseAlpha(pWarped, vec2(0.39, 0.04), vec2(0.016, 0.016), 0.01);
    ...
    float haloAlpha = 0.0;
    if (uShinyType > 0.5) {
        vec2 qHalo = (pWarped - vec2(0.08, 0.0)) / vec2(0.50, 0.13);
    ...
    float bodyRim = rimLight(pWarped, vec2(0.08, 0.0), vec2(0.50, 0.13), aBody);
    float dorsalRim = rimLight(pWarped, vec2(-0.08, 0.13), vec2(0.20, 0.05), aDorsalFin);
    float tailRim = rimLight(pWarped, vec2(-0.40, 0.0), vec2(0.10, 0.15), aTailFin);
